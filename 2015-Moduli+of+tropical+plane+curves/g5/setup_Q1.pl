# This one defines the relevant variables for g=5 and Q1^*=conv{[0,0], [4,2], [0,4]}
...
declare $genus=5;
# polytope name
declare $polytope="Q1"; # (but rotated!) Sarah: E 
...
declare $frame=new Set([0,4,12]);

# these points correspond to the cycles of the skeleton
declare $interior_lattice_points=new Array<Int>([5,6,7,9,11]);
declare %ilp_hash = ();
my $j=0; map { $ilp_hash{"$_"}=$j++ } @{$interior_lattice_points};

# group of lattice automorphisms preserving the point configuration
my $perm_action = new group::PermutationAction(GENERATORS=>[[4,3,2,1,0,7,6,5,10,9,8,11,12]]);
...
declare $triangulations_file="g5/preprocessing/E/g5E_fine_regular_affine.txt";
declare $buckets_dir="g5/preprocessing/E/g5EBuckets";

# all buckets; precomputed by Sarah
declare @all_buckets=@{load_data("$wd_path/$prefix-buckets.dat")}; # {0 2 3 4 8 9 14 15 19 33 37 53 66 67 75 84 98 106 146 169 334 344 424}
